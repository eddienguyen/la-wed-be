...
router.post('/', uploadMultiple, validateCreateGuest, async (req, res) => {
  try {
    ...
    console.log(
      '🔍 [POST /api/guests] Request received from:',
      req.headers['user-agent'],
    );
    console.log('🔍 [POST /api/guests] Origin:', req.headers.origin);
    console.log(
      '🔍 [POST /api/guests] Content-Type:',
      req.headers['content-type'],
    );
    console.log(
      '🔍 [POST /api/guests] Request body fields:',
      Object.keys(req.body),
    );
    console.log(
      '🔍 [POST /api/guests] Files received:',
    ...
    if (!imageService.isConfigured()) {
      ...
      if (req.files.invitationImageFront && req.files.invitationImageFront[0]) {
        try {
          ...
          const frontUrl = await imageService.uploadImage(
            frontFile,
            guest.id,
            venue,
            'front',
          ...
          imageUploadWarnings.push(
            `Front image upload failed: ${error.message}`,
          );
        }
      }
      ...
      if (req.files.invitationImageMain && req.files.invitationImageMain[0]) {
        try {
          ...
          const mainUrl = await imageService.uploadImage(
            mainFile,
            guest.id,
            venue,
            'main',
          ...
          imageUploadWarnings.push(
            `Main image upload failed: ${error.message}`,
          );
        }
      }
    }
    ...
    if (imageUploadWarnings.length > 0) {
      ...
      response.message =
        'Guest created successfully, but some images failed to upload';
    }
...
router.get(
  ...
  [
    ...
    query('venue')
      .optional()
      .isIn(['hue', 'hanoi'])
      .withMessage('Invalid venue filter'),
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
...
router.get(
  ...
  async (req, res) => {
    try {
      // Log detailed request information for debugging
      console.log(
        '🔍 [GET /api/guests/:id] Request received from:',
        req.headers['user-agent'],
      );
      ...
      if (!errors.isEmpty()) {
        console.error(
          '❌ [GET /api/guests/:id] Validation failed:',
          errors.array(),
        );
        return res.status(400).json({
          success: false,
...
router.patch('/:id', validateUpdateGuest, async (req, res) => {
  try {
    ...
    if (req.body.secondaryNote !== undefined)
      updates.secondaryNote = req.body.secondaryNote;
    if (req.body.invitationImageFrontUrl !== undefined)
      updates.invitationImageFrontUrl = req.body.invitationImageFrontUrl;
    if (req.body.invitationImageMainUrl !== undefined)
      updates.invitationImageMainUrl = req.body.invitationImageMainUrl;
  ...
  async (req, res) => {
    try {
      ...
      if (imageUrls.length > 0) {
        console.log(
          `🗑️ Deleting ${imageUrls.length} image(s) for guest ${id}...`,
        );
